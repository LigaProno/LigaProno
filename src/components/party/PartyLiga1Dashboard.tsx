...
function PotentialPoints({
  ht, ft, hg, ag, matchOdds,
}: {
  ht: string; ft: string; hg: string; ag: string;
  ...
  return (
    <div className="rounded-lg px-3 py-2 flex flex-wrap gap-x-4 gap-y-1 items-center" style={{ backgroundColor: "rgba(34,211,238,0.06)", borderTop: `1px solid rgba(34,211,238,0.12)` }}>
      {htPts != null && (
      ...
      {csPts != null && (
        <span className="text-xs" style={{ color: MUTED }}>
          Score <span className="text-white font-medium">{hg}-{ag}</span>
      ...
      {noOdds && (
        <span className="text-xs w-full" style={{ color: "rgba(251,146,60,0.7)" }}>
          No odds loaded — showing base points (×1). Refresh odds for real multipliers.
        </span>
      )}
    </div>
  );
